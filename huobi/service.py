...
class HuobiContract:
    ...
    def get_contract_kline(self, symbol, period, size=10):
        """
    ...
    # 获取合约当前未成交委托
    def get_contract_open_orders(self, symbol=None, page_index=None, page_size=50):
        """
        参数名称     是否必须  类型   描述
    ...
    # 获取历史成交纪录
    def get_history_orders(self, symbol,page_index=None, page_size=None,create_date=1):
        """
        symbol  true    string  品种代码        "BTC","ETH"...
...
page_size   false   int 不填默认20，不得多于50   20  """
        ...
        params["trade_type"] = 0  
        if create_date:
            params["create_date"] = int(create_date)
